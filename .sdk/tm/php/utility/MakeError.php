...
class VercelMakeError
{
    public static function call(?VercelContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new VercelContext([], null);
        }
        $op = $ctx->op ?? new VercelOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new VercelResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof VercelError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new VercelError('', $msg, $ctx);
