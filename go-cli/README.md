...
boru-driven command-line client **and** interactive REPL for the Vercel
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vercel-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vercel-cli
   ```
...
Run with no arguments to open a REPL (prompt `vercel>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/vercel-cli`. |
...
(`./vercel-cli <expr>`) and interactively (the REPL), and expressions compose the
