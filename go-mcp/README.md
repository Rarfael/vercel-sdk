...
[MCP](https://modelcontextprotocol.io) server exposing the Vercel SDK as
two agent tools — `vercel_list` and `vercel_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vercel-mcp)
make build
...
claude mcp add --scope user vercel \
  -- /absolute/path/to/vercel-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./vercel-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vercel-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user vercel \
     -- "$PWD"/dist/*/vercel-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `vercel_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `vercel_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VERCEL_APIKEY` | API key sent with every request. |
| `VERCEL_BASE` | Optional override of the API base URL. |
...
```sh
./vercel-mcp -transport http -addr :18080 &
