...
class FtCtrl extends VercelControl
{
...
class FtClient
{
    ...
    private VercelUtility $utility;

    public function __construct(VercelUtility $utility, array $options)
    {
    ...
    public function get_utility(): VercelUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (VercelContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (VercelContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = VercelConfig::make_feature($name);
    ...
    private static function buildUrl(VercelSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof VercelError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new VercelResponse($fetched);
        }
    ...
    private function fail(VercelContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (VercelContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
