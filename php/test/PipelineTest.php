...
{
    private static function utility(): VercelUtility
    {
        return new VercelUtility();
    }
    ...
    private static function ctx(array $over = []): VercelContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new VercelContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new VercelOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof VercelError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new VercelOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = VercelMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new VercelOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = VercelMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new VercelOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = VercelMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VercelSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = VercelMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new VercelResponse([]),
            'result' => new VercelResult([]),
        ]);
        [, $err] = VercelMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new VercelSpec(['step' => 's']),
            'response' => new VercelResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new VercelResult([]),
        ]);
        [, $err] = VercelMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new VercelSpec(['step' => 's']),
            'response' => new VercelResponse(self::resp(200, ['v' => 1])),
            'result' => new VercelResult([]),
        ]);
        [, $err] = VercelMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new VercelSpec(['step' => 's']),
            'response' => new VercelResponse(self::resp(200, ['v' => 2])),
            'result' => new VercelResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        VercelMakeResponse::call($ctx);
    ...
    {
        $preset = new VercelResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new VercelSpec([]),
            'response' => new VercelResponse([]),
            'result' => new VercelResult([]),
        ...
        [$got, $err] = VercelMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new VercelResult([])]);
        [, $err] = VercelMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new VercelSpec([]), 'result' => null]);
        [, $err] = VercelMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VercelSpec(['step' => 's']),
            'result' => new VercelResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new VercelOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VercelMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VercelSpec(['step' => 's']),
            'result' => new VercelResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new VercelOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VercelMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VercelResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new VercelSpec([]),
            'result' => new VercelResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = VercelMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = VercelMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new VercelError('boom', 'boom');
        $utility->fetcher = function (VercelContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VercelSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = VercelMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (VercelContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = VercelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(VercelResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (VercelContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VercelSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = VercelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new VercelResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new VercelSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = VercelMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = VercelMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = VercelMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new VercelResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, VercelDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new VercelResult(['ok' => false])]);
        $this->expectException(VercelError::class);
        VercelDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new VercelResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', VercelMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new VercelBaseFeature();
        $b = new VercelBaseFeature();
        VercelFeatureAdd::call($ctx, $a);
        VercelFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): VercelBaseFeature
    {
        $f = new VercelBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        VercelFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): VercelContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new VercelSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = VercelPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new VercelResponse(['headers' => null]),
            'result' => new VercelResult([]),
        ]);
        VercelResultHeaders::call($ctx);
