...
class VercelTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VercelTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VercelTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VercelTestRunner.getenv("VERCEL_TEST_LIVE")
        override = VercelTestRunner.getenv("VERCEL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VercelTestRunner.getenv(key)
        ...
        explain = VercelTestRunner.getenv("VERCEL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VERCEL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if VercelTestRunner._test_control is not None:
            return VercelTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VercelTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VercelTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VercelTestRunner.load_test_control()
...
def load_env_local():
    VercelTestRunner.load_env_local()


def env_override(m):
    return VercelTestRunner.env_override(m)


def entity_data(v):
    return VercelTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VercelTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VercelTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VercelTestRunner.load_test_control()


def live_delay_ms():
    return VercelTestRunner.live_delay_ms()
