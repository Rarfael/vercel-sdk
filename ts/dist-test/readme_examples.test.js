...
const TEST_SEED = { "entity": { "project": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+VercelSDK\b/.test(code) ||
        /\bVercelSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+VercelSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/VercelSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
